...
const TaskListPage: FC = () => {
   ...
   * @param e
   */
  const changeCurrent = (date: string): void => {
    setCurrent(str2Date(date));
    setCopyDate(null);
  };
   ...
   * @param e
   */
  const changeCopy = (date: string): void => {
    setCopyDate(str2Date(date));
  };
    ...
    <main className="tasks">
      <section>
        <div className="date_box">
          ...
          <input type="date" defaultValue={date2Str(current)} onChange={(e) => changeCurrent(e.target.value)} />
        </div>
        {tasks.length === 0 ? (
          <div className="history_copy">
            {copyDate ? (
              <div className="select_area">
                <div className="date_area">
                  <p className="selected_date">{copyDate.toLocaleDateString()}</p>
                  <input className="date_input" type="date" defaultValue={date2Str(copyDate)} onChange={(e) => changeCopy(e.target.value)} />
                </div>
              ...
              <div className="copy_area">
                ...
                <input className="copy_date" type="date" onChange={(e) => changeCopy(e.target.value)} />
              </div>
            )}
          </div>
        ) : null}
